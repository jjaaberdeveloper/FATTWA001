...
public class Book : MonoBehaviour {
    ...
    public List<TextMeshProUGUI> pageTexts;
    [SerializeField]
     ...
     public void UpdateSprites()
    {
        ...
        // Update Texts
        if (currentPage - 1 >= 0 && currentPage - 1 < pageTexts.Count)
        {
            pageTexts[currentPage - 1].gameObject.SetActive(true);
        }
        if (currentPage >= 0 && currentPage < pageTexts.Count)
        {
            pageTexts[currentPage].gameObject.SetActive(true);
        }

        // Disable other texts
        for (int i = 0; i < pageTexts.Count; i++)
        {
            if (i != currentPage - 1 && i != currentPage)
            {
                pageTexts[i].gameObject.SetActive(false);
            }
        }
    }
    public void TweenForward()
    {
        ...
        // Hide the current texts
        if (currentPage >= 0 && currentPage < pageTexts.Count)
        {
            pageTexts[currentPage].gameObject.SetActive(false);
        }
        if (currentPage + 1 >= 0 && currentPage + 1 < pageTexts.Count)
        {
            pageTexts[currentPage + 1].gameObject.SetActive(false);
        }
    }
    void Flip()
    {
